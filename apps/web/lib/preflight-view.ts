...
/**
 * One line per call in the simulated run: call zero is the anchor commit, the
 * rest are payments in manifest order. A failed line carries its reason, so
 * the payer can fix it rather than guess at it.
 */
export function preflightRows(
  ...
  return outcomes.map((o, i) => {
    const label = i === 0 ? "Anchor commit" : invoiceIds[i - 1]!;
